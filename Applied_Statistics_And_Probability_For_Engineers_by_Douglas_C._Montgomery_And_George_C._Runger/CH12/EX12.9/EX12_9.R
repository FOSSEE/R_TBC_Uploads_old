...
sample_var = 5.2352
x0 = matrix(c(1,8,275), nrow = 3, ncol = 1, byrow = FALSE)
y0 = t(x0)%*%beta

y0_var = sample_var * (1+((t(x0))%*%inv(X_t%*%X)%*%x0))
y0_var

t = qt(0.025,22,lower.tail = FALSE)

min = y0 - (t*sqrt(y0_var))
max = y0 + (t*sqrt(y0_var))

cat("the interval is(",min,",",max,")")

# The answer may slightly vary due to rounding off of beta values