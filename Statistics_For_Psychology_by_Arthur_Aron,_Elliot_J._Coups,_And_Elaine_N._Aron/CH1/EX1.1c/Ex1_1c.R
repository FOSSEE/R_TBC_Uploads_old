...
# Frequency Tables : Another Example

# Data on page no. 8 (bottom)
...
breaks <- seq(0, 49, by = 1)

social.interaction <- cut(social_interaction, breaks, right = F)

frequency_table <- data.frame(table(social.interaction))

View(frequency_table)   # table no. 1-5 (page no. 9)

# Note that answer is different from book for eg. [0,1) is similar to 0 and so on...
